...
bool CheckInput()
{
	...
	if (g.keyboard[SDL_SCANCODE_SPACE] == KEY_DOWN){
...
void MoveStuff()
{
	...
	if (g.keyboard[SDL_SCANCODE_O] == KEY_REPEAT){
		if (g.up == true){
			g.ship_x = g.ship_x;
			g.ship_y = g.ship_y - (3*SHIP_SPEED);
		}
		...
		if (g.left == true){
			g.ship_x = g.ship_x - (3 * SHIP_SPEED);
			g.ship_y = g.ship_y;
		}
...
void Draw()
{
	...
	if (g.keyboard[SDL_SCANCODE_T] == KEY_REPEAT){
	...
	/*
	for (int i = 0; i < g.background_width; i++){
		target.x = target.x - SCROLL_SPEED;
		SDL_RenderCopy(g.renderer, g.background, NULL, &target);
	}
	*/
	// Draw the ship --
